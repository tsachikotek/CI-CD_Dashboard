...
function parsePipelineInfo(pipelines, pipeline_url, jenkins_url, jenkinsServer) {

    var jsonPipelines = {};
    var pipeliesObj = '';

    console.log("parsePipelineInfo: " + jenkinsServer);
    ...
    var properties = pipeliesObj['property'].filter(function (obj) {
    ...
    if (typeof (properties) == 'undefined' || properties.length <= 0) {
        ...
        try { jsonPipelines['product'] = properties[0]['parameterDefinitions'][0]['defaultParameterValue']['value'] } catch { jsonPipelines['product'] = 'Missing' };
        try { jsonPipelines['version'] = properties[0]['parameterDefinitions'][1]['defaultParameterValue']['value'] } catch { jsonPipelines['version'] = 'Missing' };
        try { jsonPipelines['branch'] = properties[0]['parameterDefinitions'][2]['defaultParameterValue']['value'] } catch { jsonPipelines['branch'] = 'Missing' };
        try { jsonPipelines['build'] = properties[0]['parameterDefinitions'][3]['defaultParameterValue']['value'] } catch { jsonPipelines['build'] = 'Missing' };
    }
    //jsonPipelines[0] = {
    //    "id": pipeliesObj[0].id,
