    ...
    try {
        ...
        $(folder).each(function (index, element) {
            ...
            if (testsAutomated > 0) {
                ...
                createGaugeChart('products-stat', product, productAutoPercentage);
            }            
        ...
        if (showCoverage) {            
            ...
            createChartTestsVsAutomationPerProduct('main-stat', //(targetId + '_RegressionCoverage_percent',
            ...
            createGaugeChart('main-stat', 'All Products', autoPercentage, 3);            
        }
    }
    catch (err) {
        console.error(err);
...
function createChartTestsVsAutomationPerProduct(containerId, jsonTestsPerType, jsonCategories, xColumn, numberOfElements, max, title, subTitle, chartType, chartStacking, extraData) { //jsonTotalTests
    ...
    var chartId = insertCard(containerId, null, null, null, null, 3);
    //alert(chartId);
    var theChart = $('#' + chartId).highcharts({
        chart: {
            type: chartType, //'column'
...
function createGaugeChart(containerId, product, value, title, subTitle, greenValue, yellowValue, redValue, dataLabelFormat, maxValue,  colSpan) {
    try {
        var gaugeOptions = {
            ...
            pane: {
                ...
                endAngle: 90,
                background: {
                    backgroundColor: (Highcharts.theme && Highcharts.theme.background2) || '#ef7f7f',
                    innerRadius: '60%',
                    outerRadius: '100%',
                    shape: 'arc'
                }
            ...
            yAxis: {                
                stops: [
                    [(greenValue || 0.1), '#DF5353'], // green
                    [(yellowValue || 0.19), '#DDDF0D'], // yellow
                    [(redValue || 0.2), '#55BF3B'] // red
                ],
                lineWidth: 0,
        ...
        //if (typeof (colSpan) == 'undefined') { colSpan = 3 };
        var chartId = insertCard(containerId, null, null, null, null, (colSpan || 3));
        ...
        var chartSpeed = Highcharts.chart(chartId, Highcharts.merge(gaugeOptions, {
            yAxis: {
                ...
                title: {
                    text: '<div class"row" style="text-align:center"><span class"title">' + product + '<br>' + (title || 'Automation Coverage') + '</span></div>'
                }
            },
            ...
            series: [{
                ...
                dataLabels: {
                    format: '<div style="text-align:center"><span style="font-size:25px;color:' +
                        ((Highcharts.theme && Highcharts.theme.contrastTextColor) || 'black') + '">{y}' + (dataLabelFormat || ' %') + '</span><br/>' +
                        '<span style="font-size:12px;color:silver">' + (subTitle || 'Automated/Tests in QC') + '</span></div>'
                },
                tooltip: {
                    valueSuffix: ' %'
                }
